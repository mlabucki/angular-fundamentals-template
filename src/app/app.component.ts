...
export class AppComponent implements OnInit, OnDestroy {
  ...
  loadCharactersAndPlanet(): void {
    ...
    this.planetAndCharactersResults$ = forkJoin({
      ...
      map(({ characters, planets }) => {
        const characterObjects = characters.map((character: any) => ({
          name: character.name,
        }));
        const planetObjects = planets.map((planet: any) => ({
          name: planet.name,
        }));
        return [...characterObjects, ...planetObjects];
      })
    );
    // YOUR CODE ENDS HERE
  }
  ...
  initLoadingState(): void {
    ...
    const subscription = combineLatest({
      characters: this.mockDataService.getCharactersLoader(),
      planets: this.mockDataService.getPlanetLoader(),
    }).subscribe(
      ({ characters, planets }: { characters: boolean; planets: boolean }) => {
        this.isLoading = this.areAllValuesTrue([characters, planets]);
      }
    );
